...
-- Add data for `runners` table in `pizza_runner` schema
...
-- Add data for `customer_orders` table in `pizza_runner` schema
...
-- Add data for `runner_orders` table in `pizza_runner` schema
...
-- Add data for `pizza_names` table in `pizza_runner` schema
INSERT INTO pizza_runner.pizza_names (pizza_id, pizza_name)
VALUES
  (1, 'Meatlovers'),
  (2, 'Vegetarian');

-- Add data for `pizza_recipes` table in `pizza_runner` schema
INSERT INTO pizza_runner.pizza_recipes (pizza_id, toppings)
VALUES
  (1, '1, 2, 3, 4, 5, 6, 8, 10'),
  (2, '4, 6, 7, 9, 11, 12');

-- Add data for `pizza_toppings` table in `pizza_runner` schema
